...
export default function UserProfile() {
  ...
  const handleSelect = (selectedList) => {
    ...
    setSelectedPermissionsList((prevSelectedIds) => [
      ...new Set([...prevSelectedIds, ...selectedIds]),
    ]); // Combine with previous selected ids without duplicates
  };

  // Handle removal of items from both Domain and District dropdowns
  const handleRemove = (selectedList) => {
    const removedIds = selectedList.map((item) => item.id); // Extract only the ids to remove
    setSelectedPermissionsList((prevSelectedIds) =>
      prevSelectedIds.filter((id) => !removedIds.includes(id))
    ); // Remove deselected ids from the combined list
  };
            ...
            <React.Fragment>
              ...
              <FlexDiv gapX={20}>
                <InputContainer label="Domain List" >
                  ...
                  <Multiselect disabled style={'w-full mt-2 focus:outline-none border border-stc-purple rounded-[4px] py-3 pl-4'}
